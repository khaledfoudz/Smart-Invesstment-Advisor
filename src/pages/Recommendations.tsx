...
> = {
  Low: {
    ...
    icon: Shield,
    explanation:
      "Your profile prioritises capital preservation. These investments have lower volatility and provide stable, predictable returns — ideal for protecting what you have while growing it steadily.",
  },
  ...
  Medium: {
    ...
    icon: TrendingUp,
    explanation:
      "Your balanced profile targets a healthy mix of growth and stability. These investments accept some market fluctuation in exchange for meaningfully higher long-term returns.",
  },
  ...
  High: {
    ...
    icon: AlertTriangle,
    explanation:
      "Your aggressive profile targets maximum growth. These investments carry higher short-term volatility but historically deliver the strongest long-term returns for investors who can weather market swings.",
  },
};
...
const Recommendations = () => {
  ...
  useEffect(() => {
    const modelInput = state?.modelInput;
    ...
    const generate = async () => {
      try {
        const res = await api.post<RecommendationResult>(
  ...
  const totalScore =
    result?.investments.reduce(
      (sum, inv) => sum + Number(inv.confidencescore),
      0
    ) ?? 1;

  const riskCfg = result
    ? RISK_CONFIG[result.dbRisk] ?? RISK_CONFIG["Medium"]
    : null;

  const RiskIcon = riskCfg?.icon ?? Shield;

  const lastUpdated = result?.investments[0]?.resultsdate
    ? new Date(result.investments[0].resultsdate).toLocaleDateString(
        "en-GB",
        {
          day: "numeric",
          month: "long",
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              AI-powered analysis saved to your profile
            </p>
          </div>
            ...
            <CardContent className="flex gap-4 pt-5 pb-5">
              ...
              <div>
                <p className={`font-semibold mb-1 ${riskCfg.color}`}>
                  {riskCfg.label} risk profile ·{" "}
                  {HORIZON_LABEL[result.dbHorizon] ??
                    result.dbHorizon}
                </p>
            ...
            <CardContent className="space-y-3">
              {result.investments.map((inv) => {
                const pct = Math.round(
                  (Number(inv.confidencescore) / totalScore) * 100
                );

                return (
                  <div
                    key={inv.investmentname}
                    ...
                    <div className="flex-1 h-1.5 rounded-full bg-muted">
                      <div
                        ...
                        style={{
                          width: `${pct}%`,
                          background:
                            RISK_BAR_COLOR[inv.investmentrisk] ??
                            "#888",
                        }}
                      />
                    </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {loading
            ...
            : result?.investments.map((inv) => {
                const riskStyle =
                  RISK_CONFIG[inv.investmentrisk] ??
                  RISK_CONFIG["Medium"];
                  ...
                  >
                    <CardHeader className="pb-2">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span
                          className={`text-xs font-medium px-2 py-0.5 rounded-full border shrink-0 ${riskStyle.bg} ${riskStyle.border} ${riskStyle.color}`}
                        >
                          {riskStyle.label}
                        </span>
                      </div>
    ...
    <div className="flex items-center justify-between text-sm">
      ...
      <span
        className={`font-medium ${
          green ? "text-green-400" : "text-foreground"
        }`}
      >
        {value}
      </span>
